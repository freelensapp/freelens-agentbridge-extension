...
argument-hint: "[namespace]  # optional: refresh only this namespace"
allowed-tools: Bash(kubectl get:*), Bash(kubectl describe:*), Bash(kubectl explain:*), Bash(kubectl api-resources:*), Bash(kubectl api-versions:*), Bash(kubectl auth can-i:*), Bash(kubectl top:*), Bash(kubectl version:*), Bash(kubectl config:*), Bash(helm list:*), Bash(helm status:*), Bash(helm get:*), Bash(helm history:*), Bash(helm show:*), Bash(helm version:*), Read, Write, Edit, Task
---
...
2. **Explore namespaces in parallel.**
   - Launch **one subagent per namespace** (the generic `Task` subagent),
     running **at most 5 in parallel** (queue the rest). Each subagent is
...
4. **Write one cluster-level skill.**
   - Path: `.claude/skills/cluster-map-<cluster>/SKILL.md`.
