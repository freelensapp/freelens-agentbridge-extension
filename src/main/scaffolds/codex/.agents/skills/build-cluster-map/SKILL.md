...
description: Explore the connected Kubernetes cluster and build a durable, reusable map — a short navigation map merged into AGENTS.md plus one skill per namespace. Use when asked to map, document, or build memory of the cluster.
---
...
2. **Explore namespaces in parallel.**
   - Delegate **one subagent per namespace**, running **at most 5 in parallel**
     (queue the rest). Use the read-heavy `explorer` agent. Each subagent is
     read-only and pinned to its namespace.
...
3. **Write one skill per namespace.**
   - Path: `.agents/skills/ns-map-<namespace>/SKILL.md`.
   - Front matter:
     ```yaml
     ---
...
4. **Write one cluster-level skill.**
   - Path: `.agents/skills/cluster-map-<cluster>/SKILL.md`.
...
6. **Report** a one-paragraph summary of what was mapped (namespace count, skills
   written/updated) and remind the user the map refreshes by mentioning
   `$build-cluster-map` again, optionally naming a single namespace.

## A note on writes

Codex keeps `.agents/` read-only inside a writable workspace, so each skill file
written in steps 3 and 4 asks for approval before it lands. Approve them, or add
this workspace's `.agents` directory to `writable_roots` in `.codex/config.toml`
first — the editable settings file says how.
